...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customerName, customerPhone, customerAddress, items, subtotal, deliveryFee, total, paymentMethod, notes } = body;
    ...
    if (existing) { clientId = existing.id; }
      ...
      const { data: nc } = await supabase.from("Client").insert({ name: customerName, phone: customerPhone, platform: "website" }).select("id").single();
      clientId = nc?.id || null;
    }
